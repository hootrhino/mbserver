...
func TestInputRegistersHandler_Handle(t *testing.T) {
	handler := &InputRegistersHandler{}
	memStore := store.NewInMemoryStore().(*store.InMemoryStore)
	values := []uint16{0x1234, 0x5678}
	memStore.SetInputRegisters(values)

	request := Request{
		Frame:        []byte{0x00, 0x01, 0x00, 0x00, 0x00, 0x06, 0x01, 0x04, 0x00, 0x00, 0x00, 0x02},
		SlaveID:      0x01,
		FuncCode:     protocol.FuncCodeReadInputRegisters,
		StartAddress: 0,
		Quantity:     2,
	}
...
// TestInputRegistersHandler_Handle_Error tests error handling when getting input registers fails
func TestInputRegistersHandler_Handle_Error(t *testing.T) {
	handler := &InputRegistersHandler{}
